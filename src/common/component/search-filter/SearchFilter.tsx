...
import SearchApiService from "../../../wizard/service/SearchApiService";
import SearchInput from "../search-input/SearchInput";
import Accommodation from "../../model/Accommodation";
import {CalendarMonth, Hotel, LocalAirport, LocationCity, LocationOn, NearMe} from "@mui/icons-material";
...
class SearchFilter extends Component<any, any> {
  ...
  constructor(props: SearchFilterConfig) {
    ...
    this.state = {
      currentValue: value,
      isDestinationSelect: value.tripType.destination !== undefined
    }
  }
    ...
    return (
      <div className={"search-filter-wrapper"}>
        <div className={"search-filter-type-wrapper"}>
          { !currentDestinationSelectFlag ?
            <FormControl fullWidth size="small">
              ...
              <Select
                ...
                value={this.state.currentValue?.tripType?.category}
                label="Purpose"
              ...
              currentValue={this.state.currentValue?.tripType?.destination}
          ...
          <Link onClick={e => this.handleChangeToDestinationSelect()}>
        ...
        <FormControl fullWidth size="small">
          ...
          <Select
            ...
            <MenuItem value={PeriodType.specific}>Specific</MenuItem>
          </Select>
        </FormControl>
        ...
        <LocalizationProvider
          ...
          <DateRangePicker
            disabled={this.state.currentValue?.tripTerms?.period !== PeriodType.specific}
  ...
  private handleChangeToDestinationSelect() {
    const currentDestinationSelectFlag = this.state.isDestinationSelect;
    this.setState({ isDestinationSelect: !currentDestinationSelectFlag})
  }
  ...
  private getSearchOptionsValue() {
    const value = [];
    if (this.state.currentValue?.tripType?.accommodation === true) {
      value.push("Accommodation");
    }
    if (this.state.currentValue?.tripType?.flight === true) {
      value.push("Flight");
    }
    if (this.state.currentValue?.tripType?.transfer === true) {
      value.push("Transfer");
    }
    if (this.state.currentValue?.tripType?.carRental === true) {
  ...
  private handleDatesValueChange(dates: DateRange<any>) {
  ...
  private handleOptionsChange(value: string[]) {
    let typeValue = this.state.currentValue.tripType;
    typeValue = {
      ...typeValue,
      accommodation: value.indexOf("Accommodation") > -1
    };
    typeValue = {
      ...typeValue,
